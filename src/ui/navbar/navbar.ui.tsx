...
export default function NavbarUI({ label, link, hasBorder, closeMenu }: NavbarUIProps) {
  return (
    <Link href={link} className={`cursor-pointer w-full sm:w-1/3 flex items-center justify-center my-4 sm:my-0 ${hasBorder ? 'sm:border-r-[2px] border-ctz-light' : ''} `} onClick={() =>  closeMenu && closeMenu()}>
